...
# Function using costmap frame to calculate costs
# def calc_ref_path_follow_cost(config, traj, traj_cm_col, traj_cm_row):
#     # check row value of the reference trajectory's closest pt. This condition checks if the robot has passed the closest point.
#     # Convention: Ref path's closest pt to the robot must be in 0th position. Coordinates should be in (col, row) format.
#     if(config.vlm_ref_path[0, 0] >= int(config.costmap_shape[0]/2)): 
#         # Delete the closest point
#         np.delete(config.vlm_ref_path, 0)

#     ref_pt = config.vlm_ref_path[0] # must be only the COLUMN wrt costmap. TODO: Check this
#     traj_pt = traj_cm_col[int(math.floor(len(traj_cm_col)/2))] # TODO:check if floor function is really needed here. Also, should we choose the mid pt
#     print("Difference between traj's midpt and reference pt: ", abs(ref_pt - traj_pt))

#     # TODO: Check the magnitude of this error and set a gain accordingly.
#     return abs(ref_pt - traj_pt)
