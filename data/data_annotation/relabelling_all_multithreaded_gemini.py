            ...
            if model == "gpt":
                if config["prompt"] == "prompt_hierarchical.json" or config["prompt"] == "prompt_hierarchical_templated.json":
                    ...
                    while tries < max_tries:
                        try:
                            label, context = relabel_traj_gpt_hierarchical(imgs_base64, prompt, client, model_name, config, config["debug"])
                    ...
                    if imgs is not None: 
                        if config["use_actions"]:
                            label, context = relabel_traj_gpt(imgs_base64, prompt, client, config["model_name"], actions=actions)
                        else:
                            label, context = relabel_traj_gpt(imgs_base64, prompt, client, config["model_name"], actions=None)
...
if __name__ == "__main__":
    ...
    if config["overwrite"] and os.path.exists(config["output"]):
    ...
    print(f"Making directory {config['output']}")
