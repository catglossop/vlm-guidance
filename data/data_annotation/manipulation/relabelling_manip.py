...
def main(args):
    ...
    path_keys = [("/").join(p.split("/")[-5:]) for p in paths]
    path_keys = set(path_keys)
    remaining_path_idxs = [i for i, p in enumerate(paths) if ("/").join(p.split("/")[-5:]) not in completed_set]
    paths = [paths[i] for i in remaining_path_idxs]
    if os.path.exists(os.path.join(dataset, "pos_data.pkl")):
...
def label_trajectories(thread_num, paths, output, annotation_type, model, prompt, in_context_images, in_context_images_base64, in_context_text, period, args, current_state=None, starting_from_save=False, tqdm_func=None, global_tqdm=None):
    ...
    for path in paths:
        ...
        total_traj_len = len(glob.glob(path + "/im*.jpg"))
        print(f"Total trajectory length: {total_traj_len}")
        # with open(path + "/traj_data.pkl", "rb") as f:
        #     traj_data = pkl.load(f) 
        ...
        while i < total_traj_len:
            ...
            if annotation_type == "drawn":
                ...
                imgs = [Image.open(path + f"/im_{k}.jpg") for k in range(start, end, period)]
                if args.annotate:
                    temp_imgs = imgs
                    imgs = []
            ...
            for m in range(start, end):
                shutil.copyfile(path + f"/im_{m}.jpg", os.path.join(traj_output_dir, f"{m-start}.jpg"))
            with lock:
