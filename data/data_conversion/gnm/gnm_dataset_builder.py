...
class LCBCDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        """Define data splits."""
        return {
            'train': self._generate_examples(path='/home/noam/LLLwL/lcbc/data/data_annotation/cory_hall_labelled/*'),
    ...
    def _generate_examples(self, path) -> Iterator[Tuple[str, Any]]:
        ...
        def _parse_example(episode_path):
            ...
            samples = []
            successes = []
            # In this version, we expand out the language instructions to repeat trajectories for each language instruction
            for lang in enumerate(data['language_annotations']):
                episode = []
                language_instruction = lang['traj_description']
                language_embedding = self._embed([language_embedding])[0].numpy()
                for i in range(data['position'].shape[0]):
            ...
            return episode_path, samples, success
        ...
        episode_paths.remove("current_state.pkl")
        for name, paths in episode_paths.items():
            for sample_path in paths:
                episode_path, samples, successes = _parse_example(os.path.join(path, name, sample_path))
                for (sample, success) in zip(samples, successes):
                    if success:
