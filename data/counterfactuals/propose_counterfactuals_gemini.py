...
def main(args):
    ...
    for path in paths:
        ...
        for cf_idx, cf in enumerate(cf_json):
            ...
            if prev_atomic["end_idx"] < model_config["context_size"] - 1 or (prev_atomic["end_idx"] - model_config["context_size"] - 1) < 0:
                ...
                rollout_rotated = rollout_rotated * waypoint_spacing
                ...
                cf_old_folder = config["counterfactuals_output"].replace("cf_dataset_v3", "cf_dataset_v2") + f"/{path.split('/')[-1]}_cf_{cf_idx}"
                if os.path.exists(cf_old_folder):
                    mtimes = [time.ctime(os.path.getmtime(path)) for path in glob.glob(cf_old_folder + "/*")]
                    not_edited = ["Jan 22" not in mtime for mtime in mtimes]
                    if all(not_edited):
                        continue
    ...
    print("Generated counterfactuals for all trajectories!")
